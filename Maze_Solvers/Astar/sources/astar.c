...
static open_t *lowest_heuristic_cost(origin_t *node)
{
    open_t *tmp;
    int i = 0;
    int saved = 0;
    ...
    if (node != NULL && node->origin != NULL) {
        ...
        return (return_current(node, saved));
    }
    return (NULL);
}
    ...
    if (neighbor->x >= 0 && neighbor->x < map->largeur && neighbor->y >= 0 && neighbor->y < map->hauteur
        ...
        if ((ret = is_it_in_open(f, neighbor->y, neighbor->x)) != 1) {
    	    if (ret == -1)
    	        return (1);
    	    neighbor->closed = 0;
            // plus on se rapproche de la fin plus le cout heristique est bas
...
static int get_neighbours(map_t *map, origin_t *node, open_t *current)
{
    int i = 0;
    open_t *neighbor;
    ...
    while (i < 4) {
        ...
        if (i == 0)
            ...
            get_neighbours_pos(neighbor, current->y + 1, current->x);
        if (set_neighbours(neighbor, map, node, current) == 1)
            return (1);
        ++i;
    }
    return (0);
...
int astar(map_t *map, origin_t *node) 
{
    ...
    while (count_cells(node) > 0) {
        if ((current = lowest_heuristic_cost(node)) == NULL)
            return (-1);
